...
{
  uint32_t min=0;
  uint32_t max=((uint32_t) 1) << 16; //chux' bugfix
  while(1)
    {
       if (max <= 1 + min)
         return min;           

       uint32_t sqt = min + (max - min)/2;
       uint32_t sq = sqt*sqt;
...
int main(int argc, char* argv[])
{
	...
	//Serial version of sobel function being called.
	uint32_t serial_threshold = serial_sobel(inFile,out_file);

	//end bechmark measurement prior to writing out file
	time(&end_time);
	printf("Elapsed time for Serial Sobel Operation time (time): %f\n", difftime(end_time,start_time));
	printf("Serial Sobel Theshold: %d\n",serial_threshold);
...
{		
	...
	{	
		...
		for(i=1; i < (ht-1); i++)
		{
			for(j=1; j < (wd-1); j++)
			{
				...
				//temp = sum1 * sum1 + sum2 * sum2;		
				mag = mysqrt(sum1 * sum1 + sum2 * sum2);
				if(mag > threshold)
				{
	...
	write_bmp_file(outFile, new_bmp_img);
	free(bmp_data);
	free(new_bmp_img);
	return threshold;
}	
