	...
	__global__ void CudaSobelEdgeDetection(uint8_t *input, uint8_t *output, int height, int width, int gradientThreshold)
	{
	   ...
	   for(int i = 0; row < (height - 1); i++)
	   {
	      ...
	      double Gx = // ( * input[LINEARIZE(row - 1, col - 1, width)])
	         	+ ( +1 * input[LINEARIZE(row - 1, col + 1, width)])
	         	+ ( -1 * input[LINEARIZE(row - 1, col - 1, width)])
	         	+ ( +2 * input[LINEARIZE(row, col + 1, width)])
	        	+ ( -2 * input[LINEARIZE(row, col - 1, width)])
	         	+ ( +1  * input[LINEARIZE(row + 1, col + 1, width)])
	         	+ ( -1  * input[LINEARIZE(row + 1, col - 1, width)]);
	

	      double Gy = ( +1 * input[LINEARIZE(row - 1, col - 1, width)])
	         	+ ( +2 * input[LINEARIZE(row - 1, col, width)])
	        	+ ( +1 * input[LINEARIZE(row - 1, col + 1, width)])
	         	+ ( -1 * input[LINEARIZE(row + 1, col - 1, width)])
	         	+ ( -2 * input[LINEARIZE(row + 1, col, width)])
	         	+ ( -1 * input[LINEARIZE(row + 1, col + 1, width)]);
